...
function onGameOver()
    if not GameOverActive and not enableEnd then
        ...
        makeAnimatedLuaSprite('retry', 'Deaths/Drit-Death',400, 260);
	...
	setProperty('retry.alpha', 0);
        ...
        setPropertyFromClass('lime.app.Application', 'current.window.title', "Friday Night Funkin': Vs Flippy Flipped Out! Legacy Edition - There's no escape!") --For some reason the name of the application does not change with this script :(
end   
